...
const BacktestGraph = (graph, infoByType) => {
    ...
    const onRun = () => {
        ...
        backtestGraph({
            ...
            console.log("backtest graph succeeded")
    ...
    const downloadBtn = ButtonIcon({
        ...
        onclick: ()=> {
            DownloadJSON({
                filename: `${graph.name}_${symbol.val}_${date.val}_backtest.json`,
    ...
    const modal = ModalBackground(
        div(
            ...
            div(
                ...
                closeBtn,
                p({class: "text-lg font-medium font-bold text-center"}, "Backtest Graph"),
                div(
                    {class: "flex flex-col"},
